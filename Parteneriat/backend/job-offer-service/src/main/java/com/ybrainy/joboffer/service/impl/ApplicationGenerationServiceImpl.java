...
public class ApplicationGenerationServiceImpl implements ApplicationGenerationService {
  ...
  public ApplicationGenerationServiceImpl(
  ...
  public GenerateApplicationResponse generate(GenerateApplicationRequest request) {
    ...
    CompletableFuture<String> cvFuture =
        CompletableFuture.supplyAsync(() -> geminiService.generateOptimizedCV(cv, jobDescription, request.cvSkeleton()));
    CompletableFuture<String> letterFuture =
        CompletableFuture.supplyAsync(() -> geminiService.generateCoverLetter(cv, jobDescription));
    CompletableFuture<ProfessionalPhotoOutcome> photoFuture =
        profileBytes == null
            ? CompletableFuture.completedFuture(ProfessionalPhotoOutcome.empty())
            : CompletableFuture.supplyAsync(() -> huggingFaceService.enhanceProfilePhoto(profileBytes, profileMime));

    try {
      CompletableFuture.allOf(cvFuture, letterFuture, photoFuture).join();
    } catch (CompletionException e) {
      Throwable c = e.getCause() != null ? e.getCause() : e;
      if (c instanceof ExternalServiceException ex) {
        throw ex;
      }
      ...
      throw new ExternalServiceException("Generation failed.", c);
    }

    String optimizedCV = cvFuture.join();
    String coverLetter = letterFuture.join();
    ProfessionalPhotoOutcome photo = photoFuture.join();
