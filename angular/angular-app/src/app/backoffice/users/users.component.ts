...
export class UsersComponent {
  ...
  loadUsers(): void {
    ...
    this.http
      .get<AdminUserOverview[]>(`${this.usersApi}/admin/overview?${params.toString()}`)
  ...
  private reloadListSilently(): void {
    ...
    this.http.get<AdminUserOverview[]>(`${this.usersApi}/admin/overview?${params.toString()}`).subscribe({
      next: (data) => {
        this.users = data;
        this.refreshSelectionAfterListReload();
      },
  ...
  pendingVerificationCount(): number {
    return this.users.filter((u) => u.verificationRequired && !u.verificationApproved).length;
  }
  ...
  activeRecentlyCount(): number {
    return this.users.filter((u) => (u.activityCountLast30Days ?? 0) > 0).length;
  }
  ...
  warnedCount(): number {
    return this.users.filter((u) => (u.warningCount ?? 0) > 0).length;
  }
}
