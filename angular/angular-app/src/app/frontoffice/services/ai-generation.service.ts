...
export interface MlPredictResponse {
  ...
  color: string;
  message: string;
  probabilities: { HQ: number; LQ_CLOSE: number; LQ_EDIT: number };
  available: boolean;
}
...
export class AiGenerationService {
  ...
  /** Analyze thread quality before submission (returns full score + suggestions). */
  ...
  /** Fetch persisted AI score for an already-created thread. */
  ...
  /** ML predict: classify thread as HQ / LQ_CLOSE / LQ_EDIT using the trained scikit-learn model. */
  ...
  /** Summarize all posts of a thread using Groq AI. */
