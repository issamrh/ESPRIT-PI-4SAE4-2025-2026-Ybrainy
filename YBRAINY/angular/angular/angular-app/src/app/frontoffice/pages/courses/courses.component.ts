...
export class CoursesComponent implements AfterViewInit, OnDestroy {
  ...
  loadMLRecommendations(): void {
    const studentId = this.authMock.getUserId();
    if (!studentId) {
      ...
      next: (enrollments) => {
        ...
        if (enrollments && enrollments.length > 0) {
          ...
          if (courseIds.length > 0) {
            this.api.getById(courseIds[0]).subscribe({
              next: (course) => {
                if (course?.category) {
                  category = course.category;
                  level = course.level || 'BEGINNER';
                }
                this.fetchRecommendations(category, level);
              },
              error: () => this.fetchRecommendations(category, level)
            });
          } else {
            this.fetchRecommendations(category, level);
          }
  ...
  fetchRecommendations(category: string, level: string): void {
    this.api.getMLRecommendations(category, level, 5).subscribe({
      next: (response) => {
        this.mlRecommendations = response.recommendations || [];
  ...
  private loadPage(): void {
    this.loading = true;
    const isPublished = this.published === '' ? undefined : this.published === 'true';
    this.api.listPage({
      page: this.page,
      size: this.size,
