...
export class LessonsComponent implements OnInit, OnDestroy {
    ...
    this.sub.add(
      this.route.paramMap.subscribe((pm) => {
        ...
        if (!isNaN(id)) {
          ...
          if (this.authMock.getRole() === 'STUDENT') {
            this.api.getCourseProgress(+this.courseId, this.authMock.getUserId()).subscribe({
              next: (data) => {
                this.courseProgress = data;
                const restored = new Set<string>();
                (data.lessonProgresses ?? [])
                  .filter((lp: any) => lp.status === 'COMPLETED')
                  .forEach((lp: any) => restored.add(String(lp.lessonId)));
                this.visitedLessonIds = restored;
              },
              error: () => {} // silently ignore if not enrolled
            });
          }
          this.sub.add(
  ...
  stopLessonTimer(): void {
    if (this.lessonStartTime && this.activeTimerLessonId) {
      ...
      if (seconds > 3 && this.isStudent) {
        ...
        this.api.trackTimeSpent(courseIdNum, lessonId, this.authMock.getUserId(), seconds).subscribe();
      }
    }
  ...
  safeContentPdfUrl(item: { type: string; contentUrl: string }): SafeResourceUrl {
    ...
    const u = (raw.startsWith('http://') || raw.startsWith('https://'))
      ? raw
      : 'http://localhost:8089/api/courses/files/' + raw;
  ...
  private checkEnrollmentGate(courseId: number): void {
    const role = this.authMock.getRole();
    if (role !== 'STUDENT') {
      this.loadProgress(courseId);
      return;
    }
    const userId = this.authMock.getUserId();
    if (userId <= 0) {
  ...
  private loadProgress(courseId: number): void {
    ...
    this.api.getCourseProgress(courseId, this.authMock.getUserId()).subscribe({
      next: (progress) => {
      ...
      error: () => { /* not enrolled or no progress yet — visitedLessonIds stays empty */ },
    });
  }
  ...
  private markVisited(lesson: Lesson): void {
    const id = lesson && lesson.id != null ? String(lesson.id) : '';
    if (!id) return;
    const isStudent = this.authMock.getRole() === 'STUDENT';
    const userId = this.authMock.getUserId();
