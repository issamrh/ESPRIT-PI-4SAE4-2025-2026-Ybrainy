...
export class HeaderComponent implements AfterViewInit, OnDestroy {
  ...
  constructor(private el: ElementRef, private router: Router, private authMock: AuthMockService) {}

  get isStudent(): boolean {
    return this.authMock.getRole() === 'STUDENT';
  }
  ...
  private initStickyHeader(): void {
    function fixedHeader() {
      ...
      const scroll = $(window).scrollTop();
		const isFixed = scroll >= 10;
		if (isFixed) sticky.addClass('fixHeader');
		else sticky.removeClass('fixHeader');
		try {
			document.body.classList.toggle('fo-header-fixed', isFixed);
		} catch {
			// ignore
		}
    }
