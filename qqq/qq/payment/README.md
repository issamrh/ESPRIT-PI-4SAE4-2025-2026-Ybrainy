...
Docker (recommended for Cart/Payment/Finance + Gateway + Eureka + Observability):

1) Copy `.env.example` to `.env` and fill required secrets
2) Run: `docker compose up --build`
   - Optional: add `--profile cache --profile kafka` to start Redis/Kafka containers

Useful URLs (after compose is up):
- API Gateway: http://localhost:8092
- Eureka: http://localhost:8761
- Cart: http://localhost:8954
- Payment: http://localhost:8095
- Finance: http://localhost:8995
