...
public class StripeCheckoutService {
    ...
    public StripeCheckoutSessionResponseDTO createCheckoutSession(CartResponseDTO cart, Long userId) {
        ...
        form.add("metadata[user_id]", String.valueOf(userId));
        form.add("metadata[cart_id]", String.valueOf(cart.getId()));

        for (int i = 0; i < cart.getItems().size(); i++) {
            CartItemResponseDTO item = cart.getItems().get(i);
