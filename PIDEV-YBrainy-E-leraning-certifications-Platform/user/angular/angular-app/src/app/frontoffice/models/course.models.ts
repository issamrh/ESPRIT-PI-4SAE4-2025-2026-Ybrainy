...
export interface GazeTrackingStartRequest {
  studentId: number;
  courseId: number;
  quizId: number;
...
export interface AiReaderConfig {
  talkingHeadBaseUrl: string;
  defaultSpeakerWavPath: string;
  enabled: boolean;
}

export interface AiReaderContext {
  sourceType: 'COURSE' | 'LESSON' | string;
  courseId: number;
  lessonId?: number | null;
  title: string;
  text: string;
  talkingHeadBaseUrl: string;
  speakerWavPath?: string | null;
  speakerSource?: 'TEACHER_SAMPLE' | 'HARVARD_DEFAULT' | string | null;
}

export interface TalkingHeadSynthesisRequest {
  text: string;
  speakerWavPath?: string | null;
  speakerWavUrl?: string | null;
}

export interface TalkingHeadSynthesisResponse {
  audio_url: string;
  duration_seconds: number;
