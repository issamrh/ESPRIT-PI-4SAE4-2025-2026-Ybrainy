...
if ($Kind -eq "Maven") {
    $pomPath = Join-Path $workingDir "pom.xml"
    if (-not (Test-Path -LiteralPath $pomPath -PathType Leaf)) {
        throw "Maven project not found for $Name. Missing pom.xml at $pomPath"
    }
} elseif ($Kind -eq "Npm") {
    $packageJsonPath = Join-Path $workingDir "package.json"
    if (-not (Test-Path -LiteralPath $packageJsonPath -PathType Leaf)) {
        throw "npm project not found for $Name. Missing package.json at $packageJsonPath"
    }
}
...
switch ($Kind) {
    "Maven" {
        ...
        $runArgs = @("-nsu", "-Dmaven.test.skip=true") + $MavenArgs + @("spring-boot:run")
...
Start-Process -FilePath "powershell.exe" `
    ...
    -RedirectStandardOutput (Join-Path $workingDir "$(Get-SafeFolderName -Value $Name)-$Port.out.log") `
    -RedirectStandardError (Join-Path $workingDir "$(Get-SafeFolderName -Value $Name)-$Port.err.log") | Out-Null
