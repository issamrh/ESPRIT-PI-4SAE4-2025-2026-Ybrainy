...
export class DashboardComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private buildMlChart(): void {
    if (!this.mlCanvas || !this.data?.mlAvailable || this.data.mlTotalAnalyzed === 0) return;
    const d = this.data;
    const chart = new Chart(this.mlCanvas.nativeElement, {
      ...
      data: {
        ...
        datasets: [{
          data: [d.mlHqCount, d.mlLqEditCount, d.mlLqCloseCount],
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { callbacks: { label: (c) => ` ${c.label}: ${c.raw} threads` } }
        },
        animation: { duration: 900, easing: 'easeInOutQuart' }
      }
    });
