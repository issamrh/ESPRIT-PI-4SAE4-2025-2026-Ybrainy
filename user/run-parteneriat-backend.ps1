...
Write-Host " - Partnership Service  http://localhost:8081"
Write-Host " - Job Offer Service    http://localhost:8082"
...
Write-Host " - Port 8080 is already occupied on this machine, so the partner gateway is forced to port $GatewayPort."
...
Start-SpringBootModule -Name "Partnership Service" -WorkingDir $partnershipDir -MavenCommand $mavenCommand -Port 8081 -Arguments @("-Dmaven.repo.local=$partnershipRepoDir")
Start-SpringBootModule -Name "Job Offer Service" -WorkingDir $jobOfferDir -MavenCommand $mavenCommand -Port 8082 -Arguments @("-Dmaven.repo.local=$jobOfferRepoDir")

Wait-HttpReady -Name "Partnership Service" -Url "http://localhost:8081/actuator/health" -TimeoutSeconds 180
Wait-EurekaRegistration -AppName "partnership-service" -TimeoutSeconds 180

Wait-HttpReady -Name "Job Offer Service" -Url "http://localhost:8082/actuator/health" -TimeoutSeconds 240
Wait-EurekaRegistration -AppName "job-offer-service" -TimeoutSeconds 240

Start-SpringBootModule -Name "API Gateway" -WorkingDir $gatewayDir -MavenCommand $mavenCommand -Port $GatewayPort -Arguments @("-Dmaven.repo.local=$gatewayRepoDir") -EnvironmentVariables @{ SERVER_PORT = "$GatewayPort" }
